...
elseif strcmp(printout(end-2:end),'pdf')
    ...
    for p=1:P
        ...
        for i=1:fpp
            ...
            set(axM{(p-1)*fpp+i,1},'Position',[0.07 0.10/fpp+(fpp-i)/fpp 1-0.07-0.07 1/fpp-0.10/fpp-0.10/fpp],...
                'FontSize',8,'Title',text(0,0,[segmentStr,' ',titles{(p-1)*fpp+i}]))
            ...
            if ~isempty(axM{(p-1)*fpp+i,2})
                ...
                set(axM{(p-1)*fpp+1,2},'Position',[0.07 0.10/fpp+(fpp-i)/fpp 1-0.07-0.07 1/fpp-0.10/fpp-0.10/fpp],...
